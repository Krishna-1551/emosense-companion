...
type HighCase = {
  ...
  user_last_message_at: string | null;
...
function severityScore(c: HighCase): number {
  ...
  if (c.sentiment_score != null) {
    score += Math.round((1 - Number(c.sentiment_score)) * 15);
  }

  // keyword density
  const text = (c.flagged_excerpt || "").toLowerCase();

  const hits = NEG_KEYWORDS.reduce(
...
const Admin = () => {
  ...
  // Keep current page valid if users are added/removed.
  useEffect(() => {
  ...
  const loadAll = async () => {
    const [
      ...
      supabase.rpc("admin_emotion_distribution", {
        days: 7,
      }),
      supabase.rpc("admin_risk_trend", {
        days: 14,
      }),
      supabase.rpc("admin_high_risk_feed", {
        limit_n: 25,
      }),
      supabase.rpc("admin_user_list"),
      supabase.rpc("admin_demographics"),
      supabase.rpc("admin_high_risk_cases", {
        limit_n: 50,
      }),
  ...
  const requestConnect = async (messageId: string) => {
    ...
    toast.success(
      "Request sent. The user will be asked to consent."
    );
  ...
  const markCase = async (
    messageId: string,
    action: "follow_up" | "resolved" | "reopen"
  ) => {
      ...
      const ch = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            setSmartAlert({
              id: row.id,
              excerpt: String(row.content || "").slice(
                0,
                120
              ),
            });
            ...
            toast.warning(
              "⚠ New high-risk message detected",
              {
                duration: 6000,
              }
            );
  ...
  const riskInsight = useMemo(() => {
    ...
    const modNow = sum(recent, "moderate");
    const modPrev = sum(prev, "moderate");

    const highNow = sum(recent, "high");
    const highPrev = sum(prev, "high");
    ...
    if (
      ...
      return {
        ...
        text: `High-risk rising (${highPrev}→${highNow})`,
      };
    }
    ...
    if (
      ...
      return {
        tone: "warn" as const,
        text: `Moderate risk rising (${modPrev}→${modNow})`,
      };
    }
    ...
    if (
      ...
      return {
        tone: "good" as const,
        text: "Risk levels easing this week",
      };
    }

    return {
      tone: "neutral" as const,
      text: "Risk levels stable",
    };
  }, [risk]);
  ...
  const aiSummary = useMemo(() => {
    ...
    if (
      ...
      parts.push(
        `${dominantEmotion} dominates the past week (${pct}% of logged moods).`
      );
    }

    const risingNeg = insights.find(
      (i) =>
        [
          "sad",
          "anxious",
    ...
    if (risingNeg) {
      parts.push(
        `${risingNeg.emotion} is up ${risingNeg.pct_change}% vs last week.`
      );
    }

    if (
      riskInsight &&
      riskInsight.tone !== "neutral"
    ) {
      parts.push(
        riskInsight.text + "."
      );
    ...
    <div className="min-h-screen p-4 lg:p-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>
                EmoSense AI — analytics & monitoring
              </span>
        ...
        <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 flex items-center gap-3 animate-in slide-in-from-top-2">
          ...
          <div className="flex-1 text-sm">
            <span className="font-medium">
              New high-risk message just now.
            </span>
      ...
      <Card className="p-4 border-destructive/30">
        ...
        <p className="text-[11px] text-muted-foreground mb-3">
          Only the flagged message and basic profile
          are shown. Direct chat requires the user's
          explicit consent.
        </p>
        ...
        {visibleCases.length === 0 ? (
          ...
          <div className="space-y-3">
            {visibleCases.map((c) => {
              ...
              const droppedOff =
                !c.resolved_at &&
                c.user_message_count_24h ===
                  0 &&
                c.user_last_message_at &&
                Date.now() -
                  new Date(
                    c.user_last_message_at
                  ).getTime() >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {behavior.map((b, i) => {
              ...
              return (
                <div
                  key={i}
                  className="rounded-lg border border-border/50 p-2.5 flex items-center gap-2"
                >
                  <Badge
                    variant={m.variant}
      ...
      <Card className="p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="group flex items-center gap-2 rounded-lg px-1 py-1 -ml-1 transition-colors hover:bg-secondary/50"
            aria-expanded={usersOpen}
            aria-controls="users-list-content"
          >
            <div className="flex items-center justify-center w-7 h-7 rounded-md bg-secondary/70 group-hover:bg-secondary transition-colors">
              {usersOpen ? (
          ...
          >
            ...
            <div className="overflow-x-auto rounded-lg border border-border/40">
              <table className="w-full text-sm">
                ...
                <thead>
                  <tr className="text-left text-xs text-muted-foreground border-b border-border/50 bg-secondary/20">
